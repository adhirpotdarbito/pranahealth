...
from __lifestyle_activity_medical_report import *

json_data = {}
heart_age = 0.0
__user_risk_profile = None
range_index = {n_a:0, Low:1, Fair:2, Moderate:3, High:4, "Diabetic":5, "Renal":5, "Cardiac":5, "Hypertension":5, "Obesity":5}
...
class user_risk_profile:
    ...
    def __init__(self,id):
        ...
        self.user_liver_risk = self.__user_liver_risk() 
    ...
    # create class for liver risk
    class __user_liver_risk:
        risk = None
        liver_risk = {}
        liver_risk_reason_json = {}
...
def get_risk_factor_chd(age,sex,smoker,tc,hdl,sbp,bp_treated):
    ...
    """
    # check if user cardiac data is empty
    if tc == 0.0 or hdl == 0.0 or sbp == 0.0:
        return n_a
...
def ckd_risk_assesment(age,dia,family_history,cardiac,htn,obese,smk):
    ...
    if (family_history == one or family_history == both):
        total_point = total_point + 1
...
def diab_risk_reason_tc(age, sex, tc, dia_point, dia_risk, aging):   
    """
...
def diab_risk_reason_smoker(sex, age, smoker, dia_point, dia_risk, aging):
    """
...
def diab_risk_reason_sbp(sex, age, sbp, bp_treated, dia_point, dia_risk):
    """
...
def get_renal_points(age,dia,family_kidney,cardiac,htn,obese,smk):
    ...
    if (dia):
        total_point = total_point + 2
    # check user's family history and add points to the total_point
    if (family_kidney == one or family_kidney == both):
        total_point = total_point + 1
    # check if user is cardiacand add points to the total_point
    if (cardiac):
        total_point = total_point + 1
...
def getHighRiskIndex(risk_ls, risk_report):
    if range_index.get(risk_ls) > range_index.get(risk_report):
        return 1 
    elif range_index.get(risk_ls) < range_index.get(risk_report):
        return 2
    return 0  

def getLatestParameterDate(db, usesr_id, parameter_name):
    try:
        cursor = db.cursor()
        query = "select time_key from user_data_update where user_id=%d and parameter_name='%s'"%(int(user_id), parameter_name)
...
def getLatestDateReport(db, user_id, category_name):
    try:
        latest_date = [] 
        cursor = db.cursor()
        query = "select report_date from user_medical_reports where user_id = %d and report_type = '%s' order by report_date desc limit 1"%(int(user_id),category_name)
        cursor.execute(query)
        date = cursor.fetchone()
        if date[0] is not None:
            latest_date.append(date[0])
...
if __name__ == '__main__':
    ...
    # start user cr data
    date_report = getLatestDateReport(db, user_id, "lipid")
    date_tc = getLatestParameterDate(db, user_id, "tc_mg_dl")
    lipid_summary = getLatestReport(db, user_id, "lipid") 
    if date_report > date_tc:
        if lipid_summary.get("lipid", {}).get("total_cholesterol", {}).get("reading_value", n_a) != n_a:
            tc = float(lipid_summary.get("lipid", {}).get("total_cholesterol", {}).get("reading_value", n_a))
        else:
            tc = float(get_user_tc())
    else:
        tc = float(get_user_tc())

    date_hdl = getLatestParameterDate(db, user_id, "hdl_mg_dl")
    if date_report > date_hdl:
        if lipid_summary.get("lipid", {}).get("hdl_cholesterol", {}).get("reading_value", n_a) != n_a:
            hdl = float(lipid_summary.get("lipid", {}).get("hdl_cholesterol", {}).get("reading_value", n_a))
        else:
    ...
    whtr_risk = get_risk_whtr(sex,whtr)
    obesity = is_user_obese(sex,whtr)
    dia = is_user_diabetic()
    ...
    if htn_points == 0:
        ...
        if family_hypertension in ["one", "both"]:
    ...
    if dia_point == 0:
        ...
        dia_tc = diab_risk_reason_tc(age, sex, tc, dia_point, dia_risk, aging)
        dia_smoker = diab_risk_reason_smoker(sex, age, smoker, dia_point, dia_risk, aging)
        dia_hdl = diab_risk_reason_hdl(age, hdl, dia_point, dia_risk)
        dia_sbp = diab_risk_reason_sbp(sex, age, sbp, bp_treated, dia_point, dia_risk)
        ...
        if parent_diabetes in ["one", "both"]:
    ...
    risk_ls = get_risk_factor_diabetes(sex,age,waist,physical_activity,parent_diabetes)
    risk_report, risk_reason_report_diabetes = getMedicalReportRisk(db, user_id, "diabetes")
    lifestyle_dash_diabetes['profile'] = 'Type 2 diabetes'
    if getHighRiskIndex(risk_ls, risk_report) > 1:
        lifestyle_dash_diabetes['risk'] = risk_report
        lifestyle_dash_diabetes['reason_diabetes'] = risk_reason_report_diabetes
        __user_risk_profile.user_dia_risk.risk = risk_report
    else: 
        lifestyle_dash_diabetes['risk'] = risk_ls
        lifestyle_dash_diabetes['risk_diabetes'] = risk_reason_diabetes["Risk Reason Diabetes"]
        __user_risk_profile.user_dia_risk.risk = risk_ls
    ...
    risk_report, risk_reason_report_lipid = getMedicalReportRisk(db, user_id, "lipid")
    ...
    if risk_report != n_a:
        lifestyle_dash_chd['reason_medical_report'] = risk_reason_report_lipid
    ...
    risk_ls = ckd_risk_assesment(age,dia,family_kidney,is_user_cardiac(),is_user_hypertension(),obesity,smoker)
    risk_report, risk_reason_report_kidney = getMedicalReportRisk(db, user_id, "kidney")
    lifestyle_dash_kidney['profile'] = 'Kidney'
    if getHighRiskIndex(risk_ls, risk_report) > 1:
        lifestyle_dash_kidney['risk'] = risk_report
        lifestyle_dash_kidney['reason_renal'] = risk_reason_report_kidney
        __user_risk_profile.user_renal_risk.risk = risk_report
    else: 
        lifestyle_dash_kidney['risk'] = risk_ls
        lifestyle_dash_kidney['reason_renal'] = risk_reason_renal["Risk Reason Renal"]
        __user_risk_profile.user_renal_risk.risk = risk_ls
    ...
    risk_report, risk_reason_report_liver = getMedicalReportRisk(db, user_id, "liver")
    lifestyle_dash_liver = {}
    lifestyle_dash_liver['profile'] = 'Liver'
    lifestyle_dash_liver['risk'] = risk_report
    lifestyle_dash_liver['reason_liver_report'] = risk_reason_report_liver
    lifestyle_dash_liver['packages'] = 'Liver Function Test'
    json_data['liver'] = lifestyle_dash_liver
    __user_risk_profile.user_liver_risk.risk = risk_report
    ...
    if (duration == 'bound'):
        ...
        result_json['food_model'], result_json['food_recommendations'] = get_food_items(__user_risk_profile,int(round(recommended_cal_intake_day)),age)
    ...
    #call methods to insert medical report summary
    date_summary_lipid = getReportDateSummary(db, user_id, "lipid")
    parameter_value_tc = getParameterDateValue(date_summary_lipid,"lipid", "total_cholesterol")    
    parameter_value_hdl = getParameterDateValue(date_summary_lipid,"lipid", "hdl_cholesterol")    
    parameter_value_ldl = getParameterDateValue(date_summary_lipid,"lipid", "ldl_cholesterol")    
    parameter_value_trigly = getParameterDateValue(date_summary_lipid,"lipid", "triglycerides")    
    insertParameterDetails(user_id, "lipid_medical_report", parameter_value_tc, "total_cholesterol")
    insertParameterDetails(user_id, "lipid_medical_report", parameter_value_hdl, "hdl_cholesterol")
    insertParameterDetails(user_id, "lipid_medical_report", parameter_value_ldl, "ldl_cholesterol")
    insertParameterDetails(user_id, "lipid_medical_report", parameter_value_trigly, "triglycerides")
    date_summary_diabetes = getReportDateSummary(db, user_id, "diabetes")
    parameter_value_gf = getParameterDateValue(date_summary_diabetes,"diabetes", "glucose_fasting")    
    parameter_value_gpp = getParameterDateValue(date_summary_diabetes,"diabetes", "glucose_postprandial")    
    parameter_value_hba1c = getParameterDateValue(date_summary_diabetes,"diabetes", "hemoglobin_a1c")    
    parameter_value_mpg = getParameterDateValue(date_summary_diabetes,"diabetes", "mean_plasma_glucose")    
    insertParameterDetails(user_id, "diabetes_medical_report", parameter_value_gf, "glucose_fasting")
    insertParameterDetails(user_id, "diabetes_medical_report", parameter_value_gpp, "glucose_postprandial")
    insertParameterDetails(user_id, "diabetes_medical_report", parameter_value_hba1c, "hemoglobin_a1c")
    insertParameterDetails(user_id, "diabetes_medical_report", parameter_value_mpg, "mean_plasma_glucose")
    date_summary_kidney = getReportDateSummary(db, user_id, "kidney")
    parameter_value_bun = getParameterDateValue(date_summary_kidney,"kidney", "blood_urea_nitrogen")    
    parameter_value_creat = getParameterDateValue(date_summary_kidney,"kidney", "creatinine")    
    parameter_value_uric = getParameterDateValue(date_summary_kidney,"kidney", "uric_acid")    
    parameter_value_alb = getParameterDateValue(date_summary_kidney,"kidney", "albumin")    
    parameter_value_glob = getParameterDateValue(date_summary_kidney,"kidney", "globulin")    
    parameter_value_tp = getParameterDateValue(date_summary_kidney,"kidney", "total_protein")    
    parameter_value_bc_ratio = getParameterDateValue(date_summary_kidney,"kidney", "bun_creat_ratio")    
    insertParameterDetails(user_id, "kidney_medical_report", parameter_value_bun, "blood_urea_nitrogen")
    insertParameterDetails(user_id, "kidney_medical_report", parameter_value_creat, "creatinine")
    insertParameterDetails(user_id, "kidney_medical_report", parameter_value_uric, "uric_acid")
    insertParameterDetails(user_id, "kidney_medical_report", parameter_value_alb, "albumin")
    insertParameterDetails(user_id, "kidney_medical_report", parameter_value_glob, "globulin")
    insertParameterDetails(user_id, "kidney_medical_report", parameter_value_tp, "total_protein")
    insertParameterDetails(user_id, "kidney_medical_report", parameter_value_bc_ratio, "bun_creat_ratio")
    date_summary_liver = getReportDateSummary(db, user_id, "liver")
    parameter_value_bil_tot = getParameterDateValue(date_summary_liver,"liver", "bilirubin_total")    
    parameter_value_bil_dir = getParameterDateValue(date_summary_liver,"liver", "bilirubin_direct")    
    parameter_value_bil_indir = getParameterDateValue(date_summary_liver,"liver", "bilirubin_indirect")    
    parameter_value_alb_glob_ratio = getParameterDateValue(date_summary_liver,"liver", "albumin_globulin_ratio")    
    parameter_value_sgot = getParameterDateValue(date_summary_liver,"liver", "aspartate_aminotransferase")    
    parameter_value_ggt = getParameterDateValue(date_summary_liver,"liver", "gamma_glutamyl_transferase")    
    parameter_value_alk_phos = getParameterDateValue(date_summary_liver,"liver", "alkaline_phosphatase")    
    parameter_value_sgpt = getParameterDateValue(date_summary_liver,"liver", "alanine_transaminase")    
    insertParameterDetails(user_id, "liver_medical_report", parameter_value_bil_tot, "bilirubin_total")
    insertParameterDetails(user_id, "liver_medical_report", parameter_value_bil_dir, "bilirubin_direct")
    insertParameterDetails(user_id, "liver_medical_report", parameter_value_bil_indir, "bilirubin_indirect")
    insertParameterDetails(user_id, "liver_medical_report", parameter_value_alb_glob_ratio, "albumin_globulin_ratio")
    insertParameterDetails(user_id, "liver_medical_report", parameter_value_sgot, "aspartate_aminotransferase")
    insertParameterDetails(user_id, "liver_medical_report", parameter_value_ggt, "gamma_glutamyl_transferase")
    insertParameterDetails(user_id, "liver_medical_report", parameter_value_alk_phos, "alkaline_phosphatase")
    insertParameterDetails(user_id, "liver_medical_report", parameter_value_sgpt, "alanine_transaminase")
